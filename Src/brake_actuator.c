...
void Set_Brake(const uint8_t target_brake_val){ // 1 - brake / 0 - release brake
	...
	if(target_brake_val){ // brake
		HAL_GPIO_WritePin(BRAKE_ACTUATOR_DIRECTION_OUT_GPIO_Port, BRAKE_ACTUATOR_DIRECTION_OUT_Pin, 0); // set actuator direction
		...
		HAL_GPIO_WritePin(BRAKE_ACTUATOR_DIRECTION_OUT_GPIO_Port, BRAKE_ACTUATOR_DIRECTION_OUT_Pin, 1); // set actuator direction
